...
const getImages = async (session, prompt) => {
  ...
  const normalImageLinks = Array.from(
    new Set(imageLinks.map((link) => link.split("?w=")[0]))
  );
...
const saveImages = async (
  ...
  for (const link of links) {
    try {
      ...
      const outputPath = path.join(outputDir, `${"image"+imageNum}.jpeg`);
...
console.log(prompt_json[0].splitted_text)
var json = prompt_json[0].splitted_text;
var picture_param = "astrologique"



for(var i = 0; i < json.length; i++){
 await generateImageFiles(json[i])
